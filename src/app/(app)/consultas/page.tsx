...
interface Consulta { id: number; paciente_id: number; profissional_id: number; data: string; hora: string; observacoes: string | null; paciente_nome: string; profissional_nome: string; }
interface Paciente { id: number; nome: string; }
interface Usuario { id: number; nome: string; }

function offsetISO(n: number) { const d = new Date(); d.setDate(d.getDate() + n); return d.toISOString().split("T")[0]; }
...
export default function ConsultasPage() {
  ...
  const [form, setForm] = useState({ paciente_id: "", profissional_id: "", data: todayISO(), hora: "08:00", observacoes: "" });
  const [toast, setToast] = useState<{ msg: string; type: string } | null>(null);
  const days = Array.from({ length: 6 }, (_, i) => offsetISO(i));
  ...
  async function openModal() {
    setForm({ paciente_id: String(pacientes[0]?.id || ""), profissional_id: String(usuarios[0]?.id || ""), data: todayISO(), hora: "08:00", observacoes: "" });
    setModal(true);
  }
    ...
    <div>
      <div className="card">
        <div className="page-header">
          ...
          <button className="btn btn-lime btn-sm" onClick={openModal}>+ Agendar Consulta</button>
        </div>
        <div className="sched-wrap">
          <div className="sched-grid">
            <div className="sched-hdr">Hora</div>
            {days.map(d => { const [y, m, dd] = d.split("-"); return <div key={d} className="sched-hdr">{dd}/{m}</div>; })}
            {HOURS.map(h => (
              <>
                ...
                {days.map(d => {
                  ...
                  return (
                    <div key={d + h} className="sched-cell">
                      {c && (
                        <div className="sched-event" onClick={() => router.push(`/pacientes/${c.paciente_id}`)} title={c.paciente_nome}>
                          {c.paciente_nome.split(" ")[0]}
                          <button style={{ float: "right", background: "none", border: "none", cursor: "pointer", fontSize: 10 }} onClick={e => { e.stopPropagation(); del(c.id); }}>✕</button>
                        </div>
                      )}
                    </div>
                  );
