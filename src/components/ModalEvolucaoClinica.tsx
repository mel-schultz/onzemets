...
interface ModalEvolucaoClinicaProps {
  open: boolean;
  onClose: () => void;
  onSave: (data: EvolucaoClinicaData) => void;
  initialData?: Partial<EvolucaoClinicaData>;
  isEditing?: boolean;
}

export default function ModalEvolucaoClinica({
  open,
  onClose,
  onSave,
  initialData,
  isEditing = false,
}: ModalEvolucaoClinicaProps) {
  const [formData, setFormData] = React.useState<EvolucaoClinicaData>({
  ...
  const handleChange = (field: keyof EvolucaoClinicaData, value: string) => {
  ...
  return (
    <div className="modal-overlay open" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: "1000px", maxHeight: "90vh", overflowY: "auto" }}>
        ...
        <div className="modal-title">{isEditing ? "Editar Evolução Clínica" : "Nova Evolução Clínica"}</div>
